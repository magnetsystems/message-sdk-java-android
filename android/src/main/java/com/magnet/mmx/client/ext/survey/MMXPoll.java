...
public class MMXPoll extends SurveyQuestion<MMXPoll, MMXPollOption> {
  ...
  public static void get(String pollId, final MMX.OnFinishedListener<MMXPoll> listener) {
    getById(pollId, new MMX.OnFinishedListener<SurveyResults>() {
  ...
  public MMXPoll(String name, String question,
      List<MMXPollOption> options, Date endDate, boolean hideResultsFromOthers, Map<String, String> metaData) {
    this(null, name, User.getCurrentUserId(), question, options, endDate, hideResultsFromOthers, null,
        metaData, null, null, SurveyQuestionType.SINGLE_CHOICE);
  }

  protected MMXPoll(String pollId, String name, String ownerId, String question,
      List<MMXPollOption> options, Date endDate, boolean hideResultsFromOthers,
     MMXPollOption myVote, Map<String, String> metaData,
  ...
  //@Override MMXPoll fromSurvey(Survey survey, List<SurveyChoiceResult> results, List<SurveyAnswer> myAnswers) {
  //  MMXPoll newPoll = null;
  //
  ...
  private static MMXPoll updatePollFromSurvey(MMXPoll poll, Survey survey, List<SurveyChoiceResult> results, List<SurveyAnswer> myAnswers) {
    ...
    if(null == poll) {
      ...
      poll = new MMXPoll(survey.getId(), survey.getName(), survey.getOwners().get(0),
      ...
      for(int i = 0; i < survey.getSurveyDefinition().getQuestions().get(0).getChoices().size(); i++) {
        MMXPollOption option = poll.getOptions().get(i);
